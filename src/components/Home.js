...
const Home = () => {
  ...
  useEffect(() => {
    const handleMouseMove = (e) => setSparklePosition({ x: e.clientX, y: e.clientY });
    window.addEventListener("mousemove", handleMouseMove);
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      try {
        ...
        setCategoryData({
          new: data.filter(item => item.filter === 'new'),
          popular: data.filter(item => item.filter === 'popular'),
          featured: data.filter(item => item.filter === 'featured')
        });
      } catch (error) {
  ...
  const CategoryCard = ({ item }) => (
    <CardItem elevation={3}>
      <img
        src={item.firebaseImageUrl}
        alt={item.toolTitle}
        style={{ width: "100%", borderRadius: "10px" }}
        loading="lazy"
      />
      <Typography variant="h6" sx={{ mt: 2, mb: 1 }}>{item.toolTitle}</Typography>
      <Typography variant="body2" sx={{ px: 2 }}>
        {item.toolDescription.length > 100 ? (
          <>
            {expandedStates[item.id] ? (
              item.toolDescription
            ) : (
              `${item.toolDescription.substring(0, 100)}...`
            )}
            <Button
              variant="text"
              color="inherit"
            ...
            >
              {expandedStates[item.id] ? "Read Less" : "Read More"}
            </Button>
          </>
        ) : (
          item.toolDescription
      ...
      <Typography variant="h6" sx={{ mt: 2 }}>{item.pricingType}</Typography>
      <Typography variant="body2">{item.pricingPrice}</Typography>
      <ButtonContainer>
        <VisitButton
          variant="contained"
          href={item.visitLink}
  ...
  const CategorySection = ({ title, data }) => (
    <Box sx={{ flexGrow: 1, bgcolor: '#0e1117', overflow: 'hidden', py: 4 }}>
          ...
          <div key={index}>
            {Math.abs(activeStep - index) <= 2 && (
              <Grid
                ...
                sx={{ width: '100%', m: 0 }}
              >
                {data.slice(index, index + 4).map((item) => (
  ...
  return (
    <>
      ...
      <CategorySection title="New Category" data={categoryData.new} />
      <CategorySection title="Popular Category" data={categoryData.popular} />
      <CategorySection title="Featured Category" data={categoryData.featured} />
